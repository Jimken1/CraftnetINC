...
export function subscribeToNotifications(userId, { onInsert, onUpdate, onError } = {}) {
    ...
    let hasRetriedInitialConnection = false;
    const channel = supabase
        ...
        .subscribe((status) => {
            ...
            if (status === "CLOSED") {
                console.warn(
                    "Notification channel closed (expected during navigation/tab switches)."
                );
                return;
            }

            if (status === "CHANNEL_ERROR") {
                if (!wasEverSubscribed && !hasRetriedInitialConnection) {
                    hasRetriedInitialConnection = true;
                    console.warn(
                        "Notification channel error before first subscribe; retrying silently in 5s."
                    );
                    setTimeout(() => {
                        try {
                            channel.subscribe();
                            ...
                            console.warn(
                                "Silent notification channel retry failed:",
                                retryError
                            );
                        }
                    }, 5000);
                ...
                if (wasEverSubscribed && typeof onError === "function") {
                    const unrecoverableError = new Error("Notification channel error");
                    unrecoverableError.isUnrecoverable = true;
                    onError(unrecoverableError);
                    return;
                }

                console.warn("Notification channel transient error.");
                return;
            }

            if (status === "TIMED_OUT") {
                console.warn("Notification channel timed out; awaiting reconnect.");
                return;
            }

            console.warn("Notification channel status update:", status);
        });
