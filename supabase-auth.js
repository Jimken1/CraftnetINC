...
export async function updateDisputeStatus(disputeId, status, adminNotes = null, resolution = null) {
    try {
        ...
        const payoutResolutions = new Set(['favor_member', 'favor_apprentice', 'split', 'refund']);
        const shouldProcessPayout = status === 'resolved' && payoutResolutions.has(resolution);

        // If dispute is resolved, update job status if needed
        if (status === 'resolved' && resolution) {
            if (shouldProcessPayout) {
                // Guard 1: fast check to stop repeated requests
                const { data: payoutState, error: payoutStateError } = await supabase
                    .from("disputes")
                    .select("id, payout_processed")
                    .eq("id", disputeId)
                    .single();

                if (payoutStateError) {
                    throw new Error(`Failed to verify payout state: ${payoutStateError.message}`);
                }

                if (payoutState?.payout_processed) {
                    throw new Error("Payout already processed");
                }

                // Guard 2 (atomic): claim payout processing before any money moves
                const claimReference = `DISPUTE-PAYOUT-${disputeId}-${Date.now()}`;
                const { data: claimRows, error: claimError } = await supabase
                    .from("disputes")
                    .update({
                        payout_processed: true,
                        processed_at: new Date().toISOString(),
                        payout_reference: claimReference,
                        updated_at: new Date().toISOString()
                    })
                    .eq("id", disputeId)
                ...
                if (claimError) {
                    throw new Error(`Failed to claim dispute payout: ${claimError.message}`);
                }
            ...
            if (dispute && dispute.job_id) {
                ...
                // Ensure processed timestamp reflects completion after payout logic succeeds
                if (shouldProcessPayout) {
                    const { error: finalizePayoutError } = await supabase
                        .from("disputes")
                        .update({
                            payout_processed: true,
                            processed_at: new Date().toISOString(),
                    ...
                    if (finalizePayoutError) {
                        throw new Error(`Payout completed but failed to finalize dispute payout state: ${finalizePayoutError.message}`);
                    }
                }
            }
        }
...
export async function getApprenticeRatingDetails(apprenticeId) {
    try {
        ...
        if (error) {
            if (
                error?.code === "P0001" &&
                error?.message?.includes("Access denied: can only view own rating details")
            ) {
                return {
                    total_ratings: 0,
                    average_rating: 0.00,
