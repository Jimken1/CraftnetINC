            ...
            recommendations = await Promise.all(
                recommendations.map(async (apprentice) => {
                    try {
                        ...
                        console.warn(`Error fetching rating for apprentice ${apprentice.id}:`, error);
                        return {
                            ...apprentice,
                            rating: 0,
                            totalRatings: 0
            ...
            uniqueResults.map(async (user) => {
                if (user.role === "apprentice") {
                    try {
                        ...
                        console.warn(`Error fetching rating for apprentice ${user.id}:`, error);
                        return {
                            ...user,
                            rating: 0,
                            totalRatings: 0
        ...
        notificationState.userId,
        {
            ...
            onError: (err) => {
                if (!err?.isUnrecoverable) {
                    console.warn("Notification channel transient issue:", err);
                    return;
                }

                console.warn("Notification channel unrecoverable error:", err);
                try {
                    displayNotification({
                        type: "system",
                        title: "Live notifications unavailable",
                        message:
                            "Realtime notifications are temporarily unavailable. Refresh the page to reconnect.",
                        created_at: new Date().toISOString(),
                    });
                } catch (displayError) {
                    console.warn(
                        "Unable to show notification channel warning toast:",
                        displayError
                    );
                }
            },
